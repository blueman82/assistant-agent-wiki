...
3. **Drain-stall** — poll loop fine but one turn in flight >30 min (`DRAIN_STALL_THRESHOLD_MS`) → normal `bridge:drain-stall`. Dedup keys on `${turn_in_flight_since}:${bucket}`, not the timestamp alone — `bucket` comes from `stallBucket()` mapping elapsed time into escalation buckets `STALL_BUCKETS_MIN = [30, 60, 120, 240, 480]` (minutes), open-ended at the top. This means a stall that keeps deepening re-pings at each bucket crossing instead of being collapsed into a single first alert (a wedge stuck for 6 hours pings at 30/60/120/240min, not once at 30min). Both `DRAIN_STALL_THRESHOLD_MS` and the bucket/re-arm logic are PR #46 (`57cdf56`), the same PR that added the bridge's turn-timeout watchdog ([[capabilities/telegram-frontend]]). Checked independently of the wedge; both can fire in one tick.
