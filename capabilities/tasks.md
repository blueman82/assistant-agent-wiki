...
- **Only three infrastructure files stay tracked, plus `EXAMPLE-task.md` and `.gitkeep`** (`git ls-files tasks/`, verified 2026-07-21): the three launchd plists (listed in the `TEMPLATES` array at `scripts/install.sh:70-75`, verified 2026-07-21). **Updated 2026-07-21**: the ignore rule is now `tasks/*` — a blanket, not `tasks/*.md` — with three explicit negations (`.gitkeep`, `EXAMPLE-task.md`, `*-launchd.plist`). The plists are therefore tracked **deliberately**, not because they escaped a `.md`-only pattern. They were briefly untracked and immediately restored: a clean checkout failed 26 tests in `scripts/install.test.ts` (25 reading `template missing: .../tasks/inbox-brief-launchd.plist`) because `install.sh` reads them as templates. They carry no personal data — `install.sh:200` stamps `__REPO_PATH__` at install time and `:204-206` dies if the placeholder survives. See [[investigations/2026-07-21-repo-depersonalisation]].
