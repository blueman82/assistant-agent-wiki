...
- **Only three infrastructure files stay tracked, plus `EXAMPLE-task.md` and `.gitkeep`** (`git ls-files tasks/`, verified 2026-07-21): the three launchd plists (read by `scripts/install.sh:72-74`). They are tracked because they are `.plist`, not `.md`, so the `tasks/*.md` ignore rule never matched them — `.gitignore` carries exactly one exception line, `!tasks/EXAMPLE-task.md`, not a per-file exception.
