...
- [[capabilities/memory]] — persistent file-based fact store at `~/.rachel/memory/`, deterministically injected into the system prompt every turn via `composeSystemPrompt`; write/recall/update-over-duplicate/delete-when-wrong/self-maintenance contract; 32 KiB size guard, tail-keep since PR #62, with UTF-8-safe truncation (PRs #49-#52, hardened by #62-#64: truncation direction fix, periodic frontmatter lint, write-time PreToolUse gate; #65 write-lock CLI open, not yet merged)
...
- [[sources/2026-07-24-memory-hardening-cluster]] — PRs #62-#64 merged, #65 open/unmerged: tail-keep truncation fix, periodic frontmatter lint (30-min sweep), write-time `PreToolUse` gate (untrusted-context lockout + schema validation, three security-fix rounds against path-resolution bypasses), and a not-yet-merged O_EXCL write-lock; two explicitly unclosed gaps recorded (`mcp-exec` reachability, prompt-only lock routing)
