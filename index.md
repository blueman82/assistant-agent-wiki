...
- [[sources/2026-07-24-streaming-relay-wake-channel]] — **approved design, NOT built** (0 of 3 PRs landed, live drill pending): a mechanical mid-turn Telegram ticker (Decision A — one silent `sendMessage` then `editMessageText` edits, zero model calls) plus an answer-first reply rule, and a completion→wake channel (`~/.rachel/wake/`) that finally lets a detached job start a Rachel turn. Carries the rejected alternatives B/C/D and why they lost. Source spec is gitignored — this page is its only durable home
